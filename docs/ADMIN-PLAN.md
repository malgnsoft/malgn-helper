...
- **좌측 사이드바**: 5개 그룹 × 메뉴 (§3-2 참조)
- **상단바**: 검색 입력 / 환경 토글(개발/운영) / 사용자 메뉴(역할·로그아웃)
- **본문**: 좌측 패널 240px + 본문, 모바일은 hamburger
- **디자인 톤**: PMS와 일관 (Tailwind v4 + Pretendard + Notion-clean 라이트 모드 고정)

### 3-2. 사이드바 메뉴 정의

권한별 가시성 — `A`=관리자(`admin`) · `D`=개발자(`developer`) · `S`=상담사(`agent`).
배지(badge)는 대기·미처리 카운트 등 운영자 주의 환기용. 아이콘은 heroicons 24-outline 기준.

#### 그룹 1. 운영 보드

가장 자주 보는 작업 큐 — 사이드바 최상단.

| # | 라벨 | 경로 | 아이콘 | 배지 | A | D | S |
| --- | --- | --- | --- | --- | :-: | :-: | :-: |
| 1.1 | 홈 | `/` | `i-heroicons-home` | — | ✅ | ✅ | ✅ |
| 1.2 | 미커버 질문 | `/uncovered` | `i-heroicons-question-mark-circle` | 대기 N건 (`pending` 빈도≥3) | ✅ | ✅ | ✅ |
| 1.3 | 에스컬레이션 | `/escalations` | `i-heroicons-bell-alert` | 대기 N건 | ✅ | ✅ | ✅ |
| 1.4 | 챗봇 로그 | `/chat-logs` | `i-heroicons-chat-bubble-left-right` | 24h 부정 피드백 N | ✅ | ✅ | ✅ |
| 1.5 | Q&A 평가 | `/qa-evals` | `i-heroicons-clipboard-document-check` | 점수<3 N건 | ✅ | ✅ | ✅ |
...
| # | 라벨 | 경로 | 아이콘 | 배지 | A | D | S |
| --- | --- | --- | --- | --- | :-: | :-: | :-: |
| 2.1 | 표준답변 | `/standard-answers` | `i-heroicons-bookmark-square` | 승인 대기 N | ✅ | ✅ | ✅ |
| 2.2 | 자료 | `/materials` | `i-heroicons-document-text` | 인덱싱 실패 N | ✅ | ✅ | ✅ |
| 2.3 | 이미지 카탈로그 | `/images` | `i-heroicons-photo` | 미큐레이션 N | ✅ | ✅ | ✅ (조회만) |
| 2.4 | 토픽·서비스 카탈로그 | `/catalog` | `i-heroicons-tag` | — | ✅ | ✅ | — |

#### 그룹 3. 분석·비용

| # | 라벨 | 경로 | 아이콘 | 배지 | A | D | S |
| --- | --- | --- | --- | --- | :-: | :-: | :-: |
| 3.1 | LLM 비용 | `/cost` | `i-heroicons-currency-dollar` | 예산 ≥80% 시 경고 | ✅ | ✅ | — |
| 3.2 | 응답 품질 통계 | `/analytics/quality` | `i-heroicons-chart-bar` | — | ✅ | ✅ | — |
| 3.3 | 사용량 통계 | `/analytics/usage` | `i-heroicons-chart-pie` | — | ✅ | ✅ | — |

#### 그룹 4. 설정

| # | 라벨 | 경로 | 아이콘 | A | D | S |
| --- | --- | --- | --- | :-: | :-: | :-: |
| 4.1 | AI 설정 | `/settings/ai` | `i-heroicons-sparkles` | ✅ | ✅ | — |
| 4.2 | 안전 가드 | `/settings/safety` | `i-heroicons-shield-check` | ✅ | ✅ | — |
| 4.3 | 캐싱 | `/settings/cache` | `i-heroicons-circle-stack` | ✅ | ✅ | — |
| 4.4 | 외부 연동 | `/settings/integrations` | `i-heroicons-link` | ✅ | ✅ | — |
...
| # | 라벨 | 경로 | 아이콘 | 배지 | A | D | S |
| --- | --- | --- | --- | --- | :-: | :-: | :-: |
| 5.1 | 계정 | `/accounts` | `i-heroicons-users` | 초대 대기 N | ✅ | — | — |
| 5.2 | 감사 로그 | `/audit-logs` | `i-heroicons-clipboard-document-list` | — | ✅ | ✅ | — |
| 5.3 | API 문서 | `https://malgn-helper-api.malgnsoft.workers.dev/doc` (외부) | `i-heroicons-code-bracket` | — | ✅ | ✅ | — |
...
- **숨김 vs 비활성** — 권한 없는 메뉴는 사이드바에서 **완전히 숨김** (메뉴 표 X 표시 항목). 직접 URL 접근 시 API가 403 반환 → admin은 "권한 없음" 안내 페이지
- **배지 새로고침** — 5분마다 폴링 (`GET /admin/badges` 응답을 reactive ref에 반영) 또는 라우트 변경 시 갱신
- **그룹 헤더 클릭** — 그룹 접기/펼치기. 사용자 preference는 localStorage 저장
- **검색** — 상단바 검색은 전역 검색 (표준답변·자료·이미지·미커버 전체 LIKE) → 결과 페이지에서 카테고리별 필터

### 3-5. 신규 카탈로그 페이지 — `/catalog` (그룹 2.4)

토픽·서비스 카탈로그 한 페이지에서 통합 관리. 운영자가 자주 추가하지는 않지만 잘못된 분류 정정용으로 진입.
