...
7. **Q&A 평가 카드 폴리시 라운드** (20:10 이후)
   - **Vision으로 이미지 직접 분석** — 원본 응답의 `<img src>`를 절대URL로 변환 후 GPT-4o Vision에 첨부. LLM이 화면 내용(메뉴/버튼명)을 인지하고 적절한 캡션과 함께 답변에 배치 (이미지 있을 때만 mini → 4o 자동 업그레이드)
   - **표준답변 컨텍스트 보강** — 같은 프로젝트의 `hp_standard_answer` 최근 5건을 톤·구조 참고용으로 LLM에 전달 (복붙 금지)
   - **문단 분리 prompt 강제** — 한 `<p>`에 모든 답변을 몰아넣던 문제 해결. [인사/공감/핵심/보조/마무리]를 별도 `<p>`로 분리, 변형별 최소 `<p>` 수 명시. 검증: 짧은 답변 1 → 4 `<p>`, 긴 답변 1 → 6 `<p>`, 단계별 안내는 `<ol>` 절차로 시각화
   - **모달 UX 개선** — 분석 모달은 valid 결과(`axes.length > 0`) 도착 후에만 열기 (빈 0점 모달 노출 fix). Q&A 본문은 초기 접힌 상태로. 모달 안 🗑 삭제 → 서버 + 메모리 동기화
   - **빈 섹션 정리** — LLM이 매번 빈 배열만 채워 `후속 조치 0` 빈 박스만 보이던 followups 필드 완전 제거 (schema·prompt·UI·clipboard 4곳)
   - **`/admin/evals` 모달 즉시 열기** — 행 클릭 시 라우팅 대신 `GET /pms/evals/:id` → `QaEvalCard` 모달. db_only 폴백·빈 결과 행은 기본 목록에서 숨김 (`?includeEmpty=1`로 디버그 노출)
...
### 19:50 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `6c66221` (신규 커밋: yes)
- 메시지: feat(posts): AI 문의 답변 분석을 게시물 페이지 모달로 + 분석 로딩 + Q&A HTML 렌더

### 19:54 — `malgn-helper-api` → Cloudflare Workers
- 커밋: `fc5b701` (신규 커밋: yes)
- 메시지: feat(QaEval): D축 templates 6개로 확장 (1~3개 → 정확히 6개, maxTokens 1500→3000)

### 19:57 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `43724ce` (신규 커밋: yes)
- 메시지: feat(QaEvalCard): '평가' 라벨 → '분석' + 더보기(...) 버튼을 삭제 버튼으로 교체 + 부모에서 DELETE 처리

### 20:01 — `malgn-helper-api` → Cloudflare Workers
- 커밋: `2f3151b` (신규 커밋: yes)
- 메시지: feat(QaEval): D축 templates answer 형식·길이 가이드 강화 (인사+본문+마무리, 변형별 길이 명시)

### 20:07 — `malgn-helper-api` → Cloudflare Workers
- 커밋: `6c185f3` (신규 커밋: yes)
- 메시지: feat(QaEval): templates answer를 HTML 허용 + 원본 이미지 그대로 포함 가이드

### 20:07 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `48d4fd1` (신규 커밋: yes)
- 메시지: feat(QaAxisCard): 답변을 HTML 렌더 + fixPmsHtml로 이미지·링크 도메인 처리

### 20:10 — `malgn-helper-api` → Cloudflare Workers
- 커밋: `b37e9de` (신규 커밋: yes)
- 메시지: feat(QaEval): 원본 이미지 src 명시적 입력 + HTML 출력 강제 + 단계별 안내에 이미지 배치 가이드

### 20:25 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `48d4fd1` 재배포 (신규 커밋: no — 사용자 브라우저의 옛 chunk hash 캐시 무효화용)
- 배포: `c9d9460e.malgn-helper-pms.pages.dev`
- 메시지: (재배포) QaAxisCard 답변 HTML 렌더가 브라우저 캐시로 인해 plain text로 보이던 문제 — chunk hash 갱신

### 20:40 — `malgn-helper-api` → Cloudflare Workers
- 배포: version `e4c91fd1-fa0f-4906-9d52-71c0a88d8a2e` (커밋은 21:10 `1446679`에 일괄 포함)
- 메시지: feat(QaEval): followups 필드 제거 (LLM이 매번 빈 배열만 채워 "후속 조치 0" 빈 박스만 노출되던 문제) — schema·prompt·매핑·기본값 모두 제거

### 20:42 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `88910ba` (신규 커밋: yes)
- 배포: `bb78b5be.malgn-helper-pms.pages.dev`
- 메시지: feat(QaEval): followups 섹션 제거 — QaEvalCard·clipboard·types·example 정리

### 20:55 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `60239a4` (신규 커밋: yes)
- 배포: `d8868fac.malgn-helper-pms.pages.dev`
- 메시지: fix(posts): 분석 모달은 valid 결과 도착 후에만 열기 + Q&A 본문 초기 접기 — 빈 결과(axes=[]) 응답이 0점 모달로 보이던 문제 fix, 다시 시도 옵션 추가

### 21:10 — `malgn-helper-api` → Cloudflare Workers
- 커밋: `1446679` (신규 커밋: yes — followups 제거·Vision·표준답변 컨텍스트·문단 분리 prompt·admin/evals 필터를 한 커밋에 통합)
- 배포: 통산 4회 (`e4c91fd1` → `3893f30d` → `d9cb7e26` → `0b4c6b5a`)
- 메시지: feat(QaEval): Vision 이미지 분석 + 표준답변 컨텍스트 + 문단 분리 prompt 강화
  - llm.ts: `callOpenAiJson`에 `images?: string[]` 추가 (image_url multimodal). 이미지 있으면 자동 gpt-4o 업그레이드
  - eval/generate:
    · 원본 응답의 `<img src>`를 절대URL(`ppm.malgn.co.kr`)로 변환 후 Vision에 첨부 (최대 8장, `detail=low`)
    · 같은 프로젝트의 `hp_standard_answer` 최근 5건을 톤·구조 참고 컨텍스트로 전달 (복붙 금지)
    · `maxTokens` 6000 → 8000, `temperature` 0.2 → 0.3, `timeoutMs` 25s → 60s
  - `QA_SYSTEM_PROMPT`:
    · 이미지 처리 규칙 — 이미지마다 [안내 1~2줄 + `<img>` + 짧은 캡션(실제 메뉴/버튼명 그대로)] 묶음 강제
    · 문단 분리 강제 — 한 `<p>`에 몰아넣지 말고 [인사/공감/핵심/보조/마무리]를 별도 `<p>`로. 변형별 최소 `<p>` 수 명시
  - admin/evals: 기본 필터로 `generator='hybrid' AND overall_score IS NOT NULL` 강제 (db_only 폴백·빈 결과 행 숨김). `?includeEmpty=1`로 전체 노출 가능
- 검증 (post 149512 / id 24): 짧은 답변 `<p>=4`, 긴 답변 `<p>=6`, 단계별 안내 `<p>=6 + <ol>=1 + <li>=3` — 한 덩어리 텍스트 사라지고 의미 단위로 분리

### 21:25 — `malgn-helper-pms` → Cloudflare Pages
- 커밋: `8c794f7` (신규 커밋: yes)
- 배포: `4429f5b1.malgn-helper-pms.pages.dev`
- 메시지: feat(admin/evals): 행 클릭 시 평가 카드 모달 즉시 열기 — `$router.push` 대신 `GET /pms/evals/:id` 호출 → `QaEvalCard` 모달. 로딩 스피너 / 빈 결과 에러 모달 / 삭제·표준답변 저장 핸들러 추가
