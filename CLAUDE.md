...
- 스크립트가 자동 실행하는 4단계:
  ...
  4. `malgn-helper-mng/docs/history/history.{yyyyMMdd}.md`의 `## 배포` 섹션에 항목 append (**이력 단일 관리** — 모든 repo 배포 로그를 mng로 누적)
...
git push
pnpm deploy
# malgn-helper-mng/docs/history/history.{yyyyMMdd}.md 에 배포 항목 직접 추가 (이력 단일 관리)
```
